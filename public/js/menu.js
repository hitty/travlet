...
$ && function (d, c, h, e) {
  h.fn.menu = function (e) {
    var s = h.extend({
      ...
      onFormSuccess: function onFormSuccess(e) {}
    }, e || {}),
        i = null,
        t = function t() {
      ...
      }) : (jQuery(s.cards).removeAttr("aria-expanded"), h(s.button_cards_collapse).remove());
    },
        a = function a() {
      s.mobile ? (h("body").append(s.links_mobile_template), h(s.links_mobile_el).append(h(s.links_el, i).html()), h("body").append(s.language_mobile_template), h(s.language_mobile_el).append(h(s.language_el, i).html()), s.statement = i.html(), i.html("")) : (i.html(s.statement), s.statement = "", h(".mobile-menu").remove());
    },
        l = function l() {
      ...
      h(s.links_el).width() > t ? (s.breaks.push(h(s.links_el).width()), h(s.links_el).children().last().prependTo(h(s.hlinks)), h(s.btn).hasClass("hidden") && h(s.btn).removeClass("hidden")) : (t > s.breaks[s.breaks.length - 1] && (h(s.hlinks).children().first().appendTo(h(s.links_el)), s.breaks.pop()), s.breaks.length < 1 && (h(s.btn).addClass("hidden"), h(s.hlinks).addClass("hidden"))), h(s.btn).attr("count", s.breaks.length), h(s.links_el).width() > t && e(), h(s.links_el).css({
        opacity: 1
      });
    },
        r = function r() {
      ...
      if (0 < s.headerMsr && (e >= s.fixedHeader ? h(s.header).hasClass("fixed") || h(s.header).addClass("fixed") : h(s.header).removeClass("fixed")), e >= s.topBtnHeight ? h(s.topBtn).addClass("show") : h(s.topBtn).removeClass("show"), !s.frzScrl) {
        if (h(s.smth).eq(0).offset().top - s.headerMsr - 60 > e) return h(s.navigation_link, i).removeClass("active");

        for (var t = e, n = h(s.smth).length - 1; 0 <= n; n -= 1) {
          var a = h(s.smth).eq(n);
          if (t >= a.offset().top - s.headerMsr - 60) return h(s.navigation_link, i).removeClass("active").filter('[href="#' + a.attr("id") + '"]').addClass("active");
        }
      }
    };
    ...
    return h.fn.fitText = function (e, t) {
      ...
      return this.each(function () {
        ...
        var t = h(this);
        e(), h(d).on("resize.fittext orientationchange.fittext", e);
      });
    }, this.each(function () {
      i = h(this), function () {
        s.mobile = l(), s.mobile ? (a(), t()) : (o(), o()), h("button.toggle").each(function () {
          h(this).on("click", function () {
            h(".mobile-menu#" + h(this).data("target")).addClass("active"), h("body,html").css({
              overflow: "hidden"
            });
          });
          ...
          h(this).on("click", function (e) {
            ...
            return h("a").removeClass("on"), h(this).addClass("on"), t.removeClass("active"), h("body,html").css({
              overflow: "visible"
            }), !1;
          });
          ...
          s.mobile != l() && (s.mobile = l(), a(), t()), s.mobile || o();
        }), h(s.btn).on("click", function () {
          h(s.hlinks).toggleClass("hidden");
        }), s.fixedHeader = parseInt(h(s.header).height()), s.topBtnHeight = s.fixedHeader + 100, s.headerMsr = h(s.header).hasClass("sticky") ? parseInt(h(s.header).height()) : 0, r(), h(d).scroll(function () {
          r();
          ...
          }), h(s.navigation_link, i).removeClass("active"), h('[data-target="' + t + '"]').addClass("active " + s.reset_hover), h(this).closest(".hidden-links").length && h(this).closest("details").removeAttr("open");
        ...
        h(s.button_cards_collapse).click(function () {
          ...
          e.attr("aria-expanded", t), "false" == e.attr("aria-expanded") && h("html,body").animate({
            scrollTop: n
          }, "slow"), n = h(c).scrollTop();
        });
      }();
    });
  };
